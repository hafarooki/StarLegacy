...
object ProtectionListener : SLEventListener() {
    ...
    private fun onExplode(cancellable: Cancellable, location: Location, blocks: List<Block>) {
        if (!WorldFlags.isCivilized(location.world)
            && !isTerritoryNotUnderSiege(location)
        ) {
            return
        }
        ...
        blocks.forEach { block ->
            if (Math.random() < 0.25) {
                ...
                val dustOptions = Particle.DustOptions(Color.AQUA, 100f)
                block.world.spawnParticle(particle, particleLoc, 1, 0.0, 0.0, 0.0, 0.0, dustOptions, true)
            }
        }
    }

    private fun isTerritoryNotUnderSiege(location: Location): Boolean = Regions
        .find(location)
        .any { it is RegionTerritory && !Sieges.isUnderSiege(it.dbObjectId) }
    ...
    fun onExplosionDamage(event: EntityDamageEvent) {
        ...
        if (!WorldFlags.isFlagSet(event.entity.location.world, WorldFlags.Flag.CIVILIZED)) {
            return
        }
