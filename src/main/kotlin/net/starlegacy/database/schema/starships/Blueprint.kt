...
data class Blueprint(
    ...
    var trustedPlayerIds: MutableSet<SLPlayerId> = mutableSetOf(),
    var trustedNationIds: MutableSet<Oid<Nation>> = mutableSetOf()
) : DbObject {
    companion object : OidDbObjectCompanion<Blueprint>(Blueprint::class, setup = {
        ...
        fun create(owner: SLPlayerId, name: String, type: StarshipType, pilotLoc: Vec3i, size: Int, data: String) {
            col.insertOne(Blueprint(generateOid(), owner, name, type, pilotLoc, size, data))
        }
    ...
    fun canAccess(player: Player): Boolean {
        ...
        return slPlayerId == owner || trustedPlayerIds.contains(slPlayerId) || trustedNationIds.contains(PlayerCache[player].nationId)
    }
}
