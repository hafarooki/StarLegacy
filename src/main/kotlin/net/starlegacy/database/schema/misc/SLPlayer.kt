...
data class SLPlayer(
    ...
    var settlementId: Oid<Settlement>? = null,
    ...
    companion object : DbObjectCompanion<SLPlayer, SLPlayerId>(
        ...
        ensureIndex(SLPlayer::settlementId)
        ensureIndex(SLPlayer::nationId)
    }) {
        ...
        fun isSettlementLeader(slPlayerId: SLPlayerId): Boolean = !Settlement.none(Settlement::leaderId eq slPlayerId)

        fun isMemberOfSettlement(slPlayerId: SLPlayerId, settlementId: Oid<Settlement>): Boolean =
            matches(slPlayerId, SLPlayer::settlementId eq settlementId)

        fun isMemberOfNation(slPlayerId: SLPlayerId, nationId: Oid<Nation>): Boolean =
            matches(slPlayerId, SLPlayer::nationId eq nationId)

        private fun isSettlementLeader(sess: ClientSession, slPlayerId: SLPlayerId): Boolean =
            Settlement.col.countDocuments(sess, Settlement::leaderId eq slPlayerId) != 0L
        ...
        fun leaveSettlement(slPlayerId: SLPlayerId): Unit = trx { sess ->
            ...
            require(matches(sess, slPlayerId, SLPlayer::settlementId ne null)) { "$slPlayerId isn't in a settlement" }
            ...
            updateById(
                sess, slPlayerId, combine(
                    org.litote.kmongo.setValue(SLPlayer::settlementId, null),
                    org.litote.kmongo.setValue(
                        SLPlayer::nationId, null
                    )
                )
            )
        }
        ...
        fun joinSettlement(slPlayerId: SLPlayerId, settlementId: Oid<Settlement>): Unit = trx { sess ->
            ...
            require(matches(sess, slPlayerId, SLPlayer::settlementId eq null))

            require(Settlement.exists(sess, settlementId))

            // get the nation the settlement's in
            val nation = Settlement.col.findOneById(sess, settlementId)!!.nationId

            Settlement.updateById(sess, settlementId, pull(Settlement::invites, slPlayerId))
            updateById(
                sess, slPlayerId,
                org.litote.kmongo.setValue(SLPlayer::settlementId, settlementId),
                org.litote.kmongo.setValue(
                    SLPlayer::nationId, nation
                )
            )
        }
    }
