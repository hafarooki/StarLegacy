...
import net.starlegacy.feature.space.celestialbody.instance.PlanetInstance
 ...
 */
data class Planet(
    override val _id: Oid<Planet> = generateOid(),
    ...
    companion object : OidDbObjectCompanion<Planet>(Planet::class, setup = {
        ensureUniqueIndexCaseInsensitive(Planet::name)
        ensureUniqueIndexCaseInsensitive(Planet::planetWorld)
    }) {
        ...
        fun create(
            ...
            col.insertOne(
                Planet(generateOid(), name, sun, planetWorld, size, orbitDistance, orbitSpeed, orbitProgress, seed)
            )
        }
    ...
    @get:JsonIgnore
    val crustRadius get() = (CRUST_RADIUS_MAX * size).toInt()

    @get:JsonIgnore
    val atmosphereRadius get() = crustRadius + 3

    @delegate:JsonIgnore
    override val instance by lazy { PlanetInstance(this) }
}
