...
class SettlementZone(
    override val _id: Oid<SettlementZone> = generateOid(),
    ...
    companion object : OidDbObjectCompanion<SettlementZone>(SettlementZone::class, setup = {
        ensureIndex(SettlementZone::settlementId)
        ensureUniqueIndexCaseInsensitive(SettlementZone::name)
    }) {
        fun getByName(settlementId: Oid<Settlement>, name: String): Oid<SettlementZone>? = findOneProp(
            and(SettlementZone::settlementId eq settlementId, SettlementZone::name eq name),
            SettlementZone::_id
        )
        ...
        fun create(settlement: Oid<Settlement>, name: String, p1: Vec3i, p2: Vec3i): Oid<SettlementZone> {
            ...
            require(none(SettlementZone::name eq name))
            { "Zone named $name already exists" }

            val minPoint = Vec3i(min(p1.x, p2.x), min(p1.y, p2.y), min(p1.z, p2.z))
            val maxPoint = Vec3i(max(p1.x, p2.x), max(p1.y, p2.y), max(p1.z, p2.z))

            val id: Oid<SettlementZone> = generateOid()

            col.insertOne(SettlementZone(id, settlement, name, minPoint, maxPoint))
...
//            require(!matches(sess, zone, SettlementZone::owner ne null)) { "Zone $zone has an owner" } // zone owner can put it up for sale
        ...
        private fun <A, B : Id<A>> addTrusted(
            zoneId: Oid<SettlementZone>, id: B, property: KMutableProperty1<SettlementZone, Set<B>?>
        ): Unit = trx { sess ->
