...
interface DbObject {
    ...
    val _id: Id<out DbObject>
}

typealias Oid<T> = WrappedObjectId<T>

fun <T : DbObject> generateOid(): Oid<T> = WrappedObjectId(ObjectId())
...
abstract class DbObjectCompanion<T : DbObject, ID : Id<T>>(
    ...
    fun init() {
        col = MongoManager.getCollection(clazz).apply(setup)
    }
...
//        col.projection(property, query).firstOrNull()
    ...
    fun updateById(id: ID, vararg updates: Bson): UpdateResult =
        if (updates.size == 1) col.updateOneById(id, updates.single())
        else col.updateOneById(id, combine(*updates))

    internal fun updateById(sess: ClientSession, id: ID, vararg updates: Bson): UpdateResult =
        if (updates.size == 1) col.updateOneById(sess, id, updates.single())
        else col.updateOneById(sess, id, combine(*updates))
        ...
        MongoManager.threadPool.submit {
            while (true) {
                ...
                try {
                    ...
                    while (!StarLegacy.INITIALIZATION_COMPLETE);
