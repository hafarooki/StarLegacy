...
abstract class AutoCrafterMultiblock(
    ...
    private fun getCraftingResult(items: List<ItemStack?>?): ItemStack? {
        ...
        for ((index: Int, item: ItemStack?) in items.withIndex()) {
            val nmsItem: NMSItemStack = if (item != null) CBItemStack.asNMSCopy(item)
            else NMSItemStack.NULL_ITEM
            inventoryItems[index] = nmsItem
        }
    ...
    override fun onFurnaceTick(event: FurnaceBurnEvent, furnace: Furnace, sign: Sign) {
        ...
        val result: ItemStack = getCraftingResult(grid)?.clone() ?: run {
            return
        }
                ...
                for (ingredient: ItemStack? in grid) {
                    if (ingredient != null) {
                        ...
                        for ((index: Int, item: ItemStack?) in inputInventory.withIndex()) {
                            ...
                            if (item != null
                                && StarshipFactories.toItemString(item) == StarshipFactories.toItemString(ingredient)
