...
class MultiblockShape {
    ...
    fun checkRequirementsSpecific(origin: Block, face: BlockFace, loadChunks: Boolean, particles: Boolean): Boolean {
        return getRequirementMap(face).all { (coords, requirement) ->
            ...
            val y = coords.y
            val z = coords.z
            val relative: Block = if (loadChunks) origin.getRelative(x, y, z)
            else origin.getRelativeIfLoaded(x, y, z) ?: return@all false

            val requirementMet = requirement(relative, face)

            if (!requirementMet && particles) {
                val location = relative.location.add(0.0, 0.4, 0.0)
    ...
    class RequirementBuilder(val shape: MultiblockShape, val right: Int, val upward: Int, val inward: Int) {
        ...
        fun wireInput() = type(Wires.INPUT_COMPUTER_BLOCK)
