...
abstract class Multiblock {
    ...
    fun getBlocks(sign: Sign): List<Block> {
        val inward = sign.getFacing().oppositeFace
        val origin = sign.block.getRelative(inward)

        return getBlocks(origin, inward)
    }

    fun getBlocks(origin: Block, inward: BlockFace): List<Block> {
        val locations = shape.getLocations(inward)

        val blocks = ArrayList<Block>(1 + locations.size)

        blocks.add(origin)

        for ((dx, dy, dz) in locations) {
            blocks += origin.getRelative(dx, dy, dz)
        }
