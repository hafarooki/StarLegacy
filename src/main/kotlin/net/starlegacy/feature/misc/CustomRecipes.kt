...
import net.starlegacy.feature.multiblock.autocrafter.AutoCrafterMultiblock
...
object CustomRecipes : SLComponent() {
    ...
    fun onCraft(event: PrepareItemCraftEvent) {
        val isCustom = (event.recipe as? Keyed)?.key?.namespace == NamespacedKey(PLUGIN, "tmp").namespace

        if (isCustom) {
            return
        }

        if (event.inventory.filterNotNull().none { CustomItems[it] != null }) {
            return
        }
    ...
    private fun registerBellRecipe() {
        registerShapedRecipe(
            ...
            ingredients = mapOf(
                's' to materialChoice(Material.STICK),
                'g' to materialChoice(Material.GOLD_INGOT),
            )
        )
    }
