...
object SubsystemDetector {
    fun detectSubsystems(starship: ActiveStarship) {
        ...
        starship.powerSource = PowerSourceSubsystem(starship)
        starship.subsystems += starship.powerSource
    ...
    private fun detectSign(starship: ActiveStarship, block: Block) {
        ...
        if (sign.type.isWallSign && sign.getLine(0).toLowerCase().contains("node")) {
            ...
            for (weaponSubsystem in weaponSubsystems) {
                val nodeWeaponSets = sign.lines.slice(1..3)
                    .filter { it.isNotEmpty() }
                    .map { it.toLowerCase() }
                for (weaponSet in nodeWeaponSets) {
                    starship.weaponSets[weaponSet].add(weaponSubsystem)
                    weaponSubsystem.weaponSets.add(weaponSet)
                }
            }
            return
        }
    ...
    private fun detectThruster(starship: ActiveStarship, block: Block) {
    ...
    private fun detectWeapon(starship: ActiveStarship, block: Block) {
    ...
    private fun isDuplicate(starship: ActiveStarship, subsystem: WeaponSubsystem): Boolean {
    ...
    fun getWeaponMultiblock(block: Block, face: BlockFace): StarshipWeaponMultiblock<*>? {
        return when {
    ...
    private fun filterSubsystems(starship: ActiveStarship) {
