...
object StarshipCruising : SLComponent() {
    ...
    private fun updateCruisingShip(starship: ActivePlayerStarship) {
        ...
        starship.cruiseData.accelerate(starship.speedLimit, starship.powerSource.powerDistributor.thrusterPortion)
        ...
        if (oldVelocity.distance(velocity) > 0.01) {
            ...
            if (starship.isInterdicting) {
                starship.sendMessage("&cGravity well destabilized, deactivating")
    ...
    fun startCruising(player: Player, starship: ActivePlayerStarship) {
        ...
        val realAccel = starship.cruiseData.getRealAccel(starship.powerSource.powerDistributor.thrusterPortion)
