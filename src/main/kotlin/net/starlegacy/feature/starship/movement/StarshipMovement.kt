...
abstract class StarshipMovement(val starship: ActiveStarship, val newWorld: World? = null) {
    ...
    private fun moveCarriedShipComputers(world2: World) {
        ...
        for (data: PlayerStarship in playerShip.carriedShips.keys) {
    ...
    private fun exitPlanet(world: World, starship: ActivePlayerStarship): Boolean {
        val planet: Planet = PlanetCache.getByPlanetWorld(world) ?: return false
        val pilot: Player = starship.pilot ?: return false
        val direction: Vector = pilot.location.direction
        direction.setY(0)
        direction.normalize()

        val spaceWorld = planet.instance.spaceWorld
        if (spaceWorld == null) {
            starship.sendMessage("&cWorld ${planet.instance.spaceWorldName} not found")
            return false
        }
        ...
        val exitPoint: Location = planet.instance.position
