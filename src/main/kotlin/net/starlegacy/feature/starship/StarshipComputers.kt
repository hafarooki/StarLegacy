...
object StarshipComputers : SLComponent() {
    ...
    fun onInteract(event: PlayerInteractEvent) {
        ...
        val data: PlayerStarship? = StarshipComputers[event.player.world, block.x, block.y, block.z]
    ...
    private fun handleLeftClick(data: PlayerStarship?, player: Player, block: Block) {
        if (data == null) {
    ...
    private fun handleRightClick(data: PlayerStarship?, player: Player) {
        if (data == null) {
            return
        }
    ...
    fun onBlockBreak(event: BlockBreakEvent) {
        ...
        val computer: PlayerStarship = DeactivatedPlayerStarships[block.world, block.x, block.y, block.z]
            ?: return
        val player = event.player
    ...
    private fun tryOpenMenu(player: Player, data: PlayerStarship) {
    ...
    private val lockMap = mutableMapOf<Oid<PlayerStarship>, Any>()

    private fun getLock(id: Oid<PlayerStarship>): Any = lockMap.getOrPut(id) { Any() }

    private fun tryReDetect(player: Player, data: PlayerStarship) {
        Tasks.async {
    ...
    private fun openPilotsMenu(player: Player, data: PlayerStarship) {
        MenuHelper.apply {
                        ...
                        override fun acceptInput(context: ConversationContext, input: String?): Prompt? {
                            if (input != null) Tasks.async {
                                ...
                                if (id == null) {
                                    ...
                                    data.pilots += id
                                    PlayerStarship.updateById(data._id, addToSet(PlayerStarship::pilots, id))
                ...
                for (pilot in data.pilots) {
                    ...
                    items.add(guiButton(skullItem(pilot.uuid, name)) {
                        if (pilot != data.captain) {
                            data.pilots -= pilot
                            Tasks.async {
                                PlayerStarship.updateById(data._id, pull(PlayerStarship::pilots, pilot))
                            }
                            player.closeInventory()
    ...
    private fun openTypeMenu(player: Player, data: PlayerStarship) {
        MenuHelper.apply {
    ...
    private fun toggleLockEnabled(player: Player, data: PlayerStarship) {
