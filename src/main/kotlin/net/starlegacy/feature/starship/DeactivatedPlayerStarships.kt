...
object DeactivatedPlayerStarships : SLComponent() {
    ...
    operator fun get(world: World, x: Int, y: Int, z: Int): PlayerStarship? {
        synchronized(lock) {
    ...
    fun getLockedContaining(world: World, x: Int, y: Int, z: Int): PlayerStarship? {
        synchronized(lock) {
    ...
    fun createAsync(
        ...
        z: Int,
        playerId: UUID,
        callback: (PlayerStarship) -> Unit
    ) = Tasks.async {
        synchronized(lock) {
            require(getCache(world)[x, y, z] == null)
            val captain = playerId.slPlayerId
            val type = StarshipType.SHUTTLE
            val id = generateOid<PlayerStarship>()
            val blockKey = blockKey(x, y, z)
            val worldName = world.name
            val data = PlayerStarship(id, captain, type, worldName, blockKey)
            PlayerStarship.add(data)
    ...
    fun getSavedState(data: PlayerStarship): PlayerStarshipState? {
        return getCache(data.bukkitWorld()).savedStateCache[data].orElse(null)
    }

    fun removeState(data: PlayerStarship) {
        synchronized(lock) {
            getCache(data.bukkitWorld()).removeState(data)
        }
    }

    fun updateState(data: PlayerStarship, state: PlayerStarshipState) {
        synchronized(lock) {
            getCache(data.bukkitWorld()).updateState(data, state)
        }
    }

    fun updateType(data: PlayerStarship, newType: StarshipType) {
        data.type = newType

        Tasks.async {
            PlayerStarship.updateById(data._id, setValue(PlayerStarship::type, newType))
        }
    ...
    fun updateLockEnabled(data: PlayerStarship, newValue: Boolean) {
        data.isLockEnabled = newValue

        Tasks.async {
            PlayerStarship.updateById(data._id, setValue(PlayerStarship::isLockEnabled, newValue))
        }
    }

    fun addPilot(data: PlayerStarship, pilotID: SLPlayerId) {
        data.pilots += pilotID
        Tasks.async {
            PlayerStarship.updateById(data._id, addToSet(PlayerStarship::pilots, pilotID))
        }
    }
    ...
    private fun load(world: World) {
        ...
        PlayerStarship.find(PlayerStarship::world eq world.name).forEach { cache.add(it) }
        DEACTIVATED_SHIP_WORLD_CACHES[world] = cache
    }

    fun getSaveFile(world: World, data: PlayerStarship): File {
    ...
    fun activateAsync(
        data: PlayerStarship, state: PlayerStarshipState, carriedShips: List<PlayerStarship>,
        ...
        synchronized(lock) {
            ...
            PlayerStarship.remove(data._id)
    ...
    private fun captureCarriedShips(carriedShips: List<PlayerStarship>, cache: DeactivatedShipWorldCache)
            : MutableMap<PlayerStarship, LongOpenHashSet> {
        val carriedShipMap = mutableMapOf<PlayerStarship, LongOpenHashSet>()

        for (carried: PlayerStarship in carriedShips) {
            PlayerStarship.remove(carried._id)
    ...
    fun deactivateNow(starship: ActivePlayerStarship) {
        ...
        val carriedShipStateMap = Object2ObjectOpenHashMap<PlayerStarship, PlayerStarshipState>()

        val state: PlayerStarshipState = Tasks.getSyncBlocking {
            // this needs to be removed sync!
            ActiveStarships.remove(starship)

            for ((ship: PlayerStarship, blocks: Set<Long>) in starship.carriedShips) {
                if (!blocks.isEmpty()) {
    ...
    private fun saveDeactivatedData(
        ...
        carriedShipStateMap: Object2ObjectOpenHashMap<PlayerStarship, PlayerStarshipState>
    ) {
        synchronized(lock) {
            val cache: DeactivatedShipWorldCache = getCache(world)

            val data: PlayerStarship = starship.data
            ...
            for (carried: PlayerStarship in starship.carriedShips.keys) {
                carried.containedChunks = null
                cache.add(carried)
                carriedShipStateMap[carried]?.let { carriedDataState: PlayerStarshipState ->
                    cache.updateState(carried, carriedDataState)
                }
                PlayerStarship.add(carried)
            }
        }
    }

    fun destroyAsync(data: PlayerStarship, callback: () -> Unit = {}): Unit = Tasks.async {
        synchronized(lock) {
            destroy(data)
    ...
    fun destroyManyAsync(data: List<PlayerStarship>, callback: () -> Unit = {}): Unit = Tasks.async {
        synchronized(lock) {
            for (data in data) {
                destroy(data)
            }
    ...
    private fun destroy(data: PlayerStarship) {
