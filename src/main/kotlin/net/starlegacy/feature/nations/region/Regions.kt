...
import net.starlegacy.feature.nations.region.types.*
...
object Regions : SLComponent() {
    ...
    override fun onEnable() {
        registerRegionType(Settlement.Companion) { RegionSettlement(it) }
        ...
        registerRegionType(NationOutpost.Companion) { RegionNationOutpost(it) }
        ...
        plugin.listen<PlayerChangedWorldEvent> { event ->
            val player = event.player
        ...
        SettlementRole.watchUpdates { change ->
            ...
            if (change.containsUpdated(SettlementRole::permissions) || change.containsUpdated(SettlementRole::members)) {
                ...
                refreshSettlementLocally(settlementId)
            }
        }
    ...
    fun refreshSettlementLocally(settlementId: Oid<Settlement>) {
        locked {
            cache.get<Region<Settlement>>(settlementId)?.refreshAccessCache()
        }
    }
    ...
    fun refreshSettlementMembersLocally(settlementId: Oid<Settlement>) {
        locked {
            for (player in plugin.server.onlinePlayers) {
                if (PlayerCache[player].settlementId != settlementId) {
                    continue
                }
    ...
    private inline fun <reified A : DbObject, reified B : Region<A>> registerRegionType(
        objectCompanion: OidDbObjectCompanion<A>, crossinline createNew: (A) -> B
    ) {
...
private class RegionCache {
    ...
    fun add(region: Region<*>) {
        idMap[(region.dbObjectId as WrappedObjectId<*>).id] = region
    ...
    fun remove(region: Region<*>) {
        idMap.remove((region.dbObjectId as WrappedObjectId<*>).id)
