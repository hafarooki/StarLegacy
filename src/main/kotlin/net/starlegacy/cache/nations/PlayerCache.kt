...
object PlayerCache : ManualCache() {
    ...
    override fun load() {
        ...
        fun cache(id: SLPlayerId, data: SLPlayer) {
            val settlement: Oid<Settlement>? = data.settlementId
            val nation: Oid<Nation>? = data.nationId
        ...
        SLPlayer.watchUpdates { change ->
            ...
            if (change.containsUpdated(SLPlayer::settlementId) || change.containsUpdated(SLPlayer::nationId)) {
                Tasks.async {
        ...
        fun recalculateTags(): Unit = synchronized(mutex) {
            for ((id, data) in PLAYER_DATA) {
                ...
                data.settlementId?.let { data.settlementTag = SettlementRole.getTag(slPlayerId) }
                data.nationId?.let { data.nationTag = NationRole.getTag(slPlayerId) }
            }
        }
