...
internal abstract class MoneyCommand<Parent : MoneyHolder> : SLCommand() {
    abstract fun requireCanDeposit(sender: Player, parent: Parent)
    abstract fun requireCanWithdraw(sender: Player, parent: Parent)

    abstract fun requireDefaultParent(sender: Player): Parent

    abstract fun resolveParent(name: String): Parent

    abstract fun getBalance(parent: Parent): Int

    abstract fun deposit(parent: Parent, amount: Int)
    abstract fun withdraw(parent: Parent, amount: Int)

    private fun requireParent(sender: Player, suppliedName: String?): Parent =
    ...
    open fun onBalance(sender: Player, @Optional parentName: String?) = asyncCommand(sender) {
        val parent: Parent = when (parentName) {
    ...
    open fun onDeposit(sender: Player, amount: Int) = asyncCommand(sender) {
        val parent: Parent = requireDefaultParent(sender)
