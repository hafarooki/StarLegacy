...
internal object SettlementZoneCommand : SLCommand() {
    ...
    private fun getZones(settlement: Settlement): List<RegionSettlementZone> {
        return Regions.getAllOf<RegionSettlementZone>()
            .filter { it.settlementId == settlement._id }
    }

    private fun requireSettlementWithPermission(sender: Player): Settlement {
    ...
    fun onCreate(sender: Player, name: String) = asyncCommand(sender) {
        val settlement: Settlement = requireSettlementWithPermission(sender)

        validateName(name)

        val region: RegionSettlement = Regions[settlement._id]

        val loc = sender.location
        failIf(region.world != loc.world.name || !region.contains(loc.blockX, loc.blockY, loc.blockZ))
        { "You're can only make zones inside of your settlement's territory" }

        val (pos1: Vec3i, pos2: Vec3i) = validateSelection(sender, settlement)

        SettlementZone.create(settlement._id, name, pos1, pos2)
    ...
    private fun validateSelection(sender: Player, settlement: Settlement): Pair<Vec3i, Vec3i> {
        ...
        val minZ = min(pos2.z, pos2.z)
        val maxZ = max(pos2.z, pos2.z)

        for (chunkX in (minX shr 4)..(maxX shr 4)) {
            for (chunkZ in (minZ shr 4)..(maxZ shr 4)) {
                failIf(!settlement.chunks.contains(chunkKey(chunkX, chunkZ))) {
                    "Chunk $chunkX, $chunkZ is not claimed by the settlement"
                }
            }
        }
    ...
    fun onList(sender: Player) = asyncCommand(sender) {
        ...
        MenuHelper.apply {
            ...
            Tasks.sync {
                sender.openPaginatedMenu("${settlement.name} Zones", items)
            }
        }
    }

    /** This should theoretically be handled by the context resolver, but.. just in case ;)*/
    private fun requireSettlementWithPermissionAndZone(sender: Player, zone: RegionSettlementZone): Settlement {
        val settlement = requireSettlementWithPermission(sender)
        failIf(settlement._id != zone.settlementId) {
            "Zone ${zone.name} isn't in your settlement!"
        }
        return settlement
    }
    ...
    fun onDelete(sender: Player, zone: RegionSettlementZone) = asyncCommand(sender) {
        ...
        SettlementZone.delete(zone.dbObjectId)
    ...
    private fun getZoneData(zone: RegionSettlementZone): SettlementZone = SettlementZone.findById(zone.dbObjectId)
    ...
    fun onSetPrice(sender: Player, zone: RegionSettlementZone, price: Int) = asyncCommand(sender) {
        ...
        SettlementZone.setPrice(zone.dbObjectId, newPrice)
        sender msg when {
    ...
    fun onSetRent(sender: Player, zone: RegionSettlementZone, rent: Int) = asyncCommand(sender) {
        ...
        SettlementZone.setRent(zone.dbObjectId, newRent)
        sender msg when {
    ...
    fun onReclaim(sender: Player, zone: RegionSettlementZone) = asyncCommand(sender) {
        ...
        SettlementZone.setOwner(zone.dbObjectId, null)

        sender msg "&aReclaimed region ${zone.name} from ${getPlayerName(owner)}"

        val message = "&7${sender.name} reclaimed your plot ${zone.name} in ${settlement.name}"
