...
internal object SettlementRoleCommand : RoleCommand<Settlement, SettlementRole.Permission, SettlementRole>() {
    ...
    override fun requireParent(sender: Player): Settlement {
        return requireSettlementIn(sender)
    }

    override fun requirePermission(sender: Player, parent: Settlement, permission: SettlementRole.Permission) {
        requireSettlementPermission(sender, parent, permission)
    }

    override fun isLeader(slPlayerId: SLPlayerId, parent: Settlement): Boolean {
        return parent.leaderId == slPlayerId
    }

    override fun isMember(slPlayerId: SLPlayerId, parent: Settlement): Boolean {
        return SLPlayer.isMemberOfSettlement(slPlayerId, parent._id)
    }

    override fun getMembers(parent: Settlement): List<SLPlayerId> {
        return Settlement.getMembers(parent._id).toList()
    }
