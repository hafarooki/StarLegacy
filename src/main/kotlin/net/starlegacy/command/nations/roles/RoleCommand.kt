...
internal abstract class RoleCommand<Parent : DbObject, Permission : Enum<Permission>, T : Role<Parent, Permission>>
    ...
    protected abstract fun requireParent(sender: Player): Parent

    protected abstract fun requirePermission(sender: Player, parent: Parent, permission: Permission)

    protected abstract fun isLeader(slPlayerId: SLPlayerId, parent: Parent): Boolean

    protected abstract fun isMember(slPlayerId: SLPlayerId, parent: Parent): Boolean

    protected abstract fun getMembers(parent: Parent): List<SLPlayerId>

    private fun requireManageableRole(sender: Player, parent: Parent, name: String): T {
        val role = roleCompanion.findOne(and(roleCompanion.parentProperty eq parent._id, roleCompanion.nameQuery(name)))
    ...
    private fun requireManageablePlayer(sender: Player, parent: Parent, playerName: String): SLPlayerId {
        val id: UUID = resolveOfflinePlayer(playerName)
        val slPlayerId = id.slPlayerId

        failIf(!isMember(slPlayerId, parent))
        { "$playerName is not a member" }
    ...
    private fun getWeight(slPlayerId: SLPlayerId, parent: Parent): Int = when {
    ...
    private fun validateWeight(sender: Player, weight: Int, parent: Parent) {
        val playerWeight = getWeight(sender.slPlayerId, parent)
        failIf(weight >= playerWeight)
        { "You can't sent the weight of a role to greater than or equal to your own weight ($playerWeight)!" }

        failIf(weight !in 0..1000)
        { "Weight must be between 0 and 1000 (higher = more authority, leader is always 1001)" }
    }

    private fun validateName(parent: Parent, name: String) {
        failIf(!name.isAlphanumeric())
        { "Name must only use numbers and letters" }
        ...
        failIf(
            !roleCompanion.none(
                and(
                    roleCompanion.parentProperty eq parent._id,
                    roleCompanion.nameQuery(name)
                )
            )
        ) { "A role with that name already exists" }
    }

    private fun requireCanManage(sender: Player, other: SLPlayerId, parent: Parent) {
        if (isLeader(sender.slPlayerId, parent)) {
            return
        }
        failIf(getWeight(sender.slPlayerId, parent) <= getWeight(other, parent))
        { "${SLPlayer.getName(other)}'s weight is higher than yours, so you can't manage them." }
    }

    private fun requireManageableParent(sender: Player): Parent {
    ...
    open fun onManage(sender: Player) = asyncCommand(sender) {
        val parent: Parent = requireManageableParent(sender)

        val roleItems = roleCompanion.find(roleCompanion.parentProperty eq parent._id)
    ...
    open fun onCreate(sender: Player, name: String, color: SLTextStyle, weight: Int) = asyncCommand(sender) {
        ...
        @Suppress("UNCHECKED_CAST")
        val parentId = parent._id as Oid<Parent>

        failIf(roleCompanion.count(roleCompanion.parentProperty eq parentId) >= 27)
        { "You can only make 27 roles." }
        ...
        roleCompanion.create(parentId, name, color, weight)
    ...
    open fun onPermissionAdd(sender: Player, role: String, permission: Permission) = asyncCommand(sender) {
        ...
        failIf(roleData.permissions.contains(permission))
        { "Role ${roleData.name} already has permission $permission!" }
    ...
    open fun onPermissionList(sender: Player, role: String) = asyncCommand(sender) {
        ...
        failIf(permissions.isEmpty())
        { "${roleData.name} has no permissions. To add permissions, use /$name permission add <permission>." }
    ...
    open fun onPermissionRemove(sender: Player, role: String, permission: Permission) {
        ...
        failIf(!roleData.permissions.contains(permission))
        { "${roleData.name} doesn't have the $permission permission!" }
    ...
    open fun onMemberGUI(sender: Player, player: String) = asyncCommand(sender) {
        ...
        val allRoles = roleCompanion.find(roleCompanion.parentProperty eq parent._id)
            .sortedByDescending { it.weight }
            .map { it.name }
    ...
    open fun onMemberAdd(sender: Player, player: String, role: String) = asyncCommand(sender) {
        ...
        failIf(roleCompanion.getRoles(playerId).contains(getId(roleData)))
        { "$player already has role ${roleData.name}." }
