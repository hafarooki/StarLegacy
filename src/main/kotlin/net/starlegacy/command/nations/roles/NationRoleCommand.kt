...
internal object NationRoleCommand : RoleCommand<Nation, NationRole.Permission, NationRole>() {
    ...
    override fun requireParent(sender: Player): Nation {
        return requireNationIn(sender)
    }

    override fun requirePermission(sender: Player, parent: Nation, permission: NationRole.Permission) {
        requireNationPermission(sender, parent, permission)
    }

    override fun isLeader(slPlayerId: SLPlayerId, parent: Nation): Boolean {
        return SettlementCache[parent.capitalId].leaderId == slPlayerId
    }

    override fun isMember(slPlayerId: SLPlayerId, parent: Nation): Boolean {
        return SLPlayer.isMemberOfNation(slPlayerId, parent._id)
    }

    override fun getMembers(parent: Nation): List<SLPlayerId> {
        return Nation.getMembers(parent._id).toList()
    }
