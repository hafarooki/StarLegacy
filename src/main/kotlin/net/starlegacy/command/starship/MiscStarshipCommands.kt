...
object MiscStarshipCommands : SLCommand() {
    ...
    fun onJump(sender: Player, planet: String) {
        ...
        val cachedPlanet = PlanetCache.getByName(planet)
        ...
        if (cachedPlanet.instance.spaceWorld != sender.world) {
            sender msg "&c$planet is not in this space sector."
            return
        }

        val x = cachedPlanet.instance.position.x
        val z = cachedPlanet.instance.position.z
    ...
    fun onSetTarget(sender: Player, set: String, @Optional player: OnlinePlayer?) {
        ...
        failIf(starship !is ActivePlayerStarship) {
            "Cannot set targets on non player ships"
        }
        starship as ActivePlayerStarship
