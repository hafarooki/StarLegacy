...
object BlueprintCommand : SLCommand() {
    ...
    private fun getMaxBlueprints(player: Player): Int {
        return player.effectivePermissions.asSequence()
            .filter { it.permission.startsWith("starships.blueprints.") }
            .mapNotNull { it.permission.removePrefix("starships.blueprints.").toIntOrNull() }
            .maxOrNull() ?: 0
    }
    ...
    private fun blueprintInfo(blueprint: Blueprint): List<String> {
        ...
        if (blueprint.trustedNationIds.isNotEmpty()) {
            list.add("&7Trusted Players&8: &b${blueprint.trustedPlayerIds.joinToString { getPlayerName(it) }}}")
            list.add("&7Trusted Nations&8: &a${blueprint.trustedNationIds.joinToString { NationCache[it].name }}")
        }
        return list
    }
    ...
    fun onTrustPlayer(sender: Player, name: String, player: String) {
        ...
        failIf(blueprint.trustedPlayerIds.contains(slPlayerId)) {
            "$player is already trusted, you might be looking for /blueprint untrust player $name $player"
        }
        blueprint.trustedPlayerIds.add(slPlayerId)
        saveBlueprint(blueprint)
    ...
    fun onUntrustPlayer(sender: Player, name: String, player: String) {
        ...
        blueprint.trustedPlayerIds.remove(slPlayerId)
        saveBlueprint(blueprint)
    ...
    fun onTrustNation(sender: Player, name: String, nationName: String) {
        val blueprint = getBlueprint(sender, name)
        val nation = resolveNation(nationName)
        failIf(blueprint.trustedNationIds.contains(nation._id)) {
            "$nation is already trusted, you might be looking for /blueprint untrust nation $name $nation"
        }
        blueprint.trustedNationIds.add(nation._id)
        saveBlueprint(blueprint)
    ...
    @CommandCompletion("@blueprints @nations")
    fun onUntrustNation(sender: Player, name: String, nationName: String) {
        val blueprint = getBlueprint(sender, name)
        val nation = resolveNation(nationName)
        failIf(!blueprint.trustedNationIds.contains(nation._id)) {
            "$nationName is not trusted, you might be looking for /blueprint trust nation $name $nationName"
        }
        blueprint.trustedNationIds.remove(nation.component1())
        saveBlueprint(blueprint)
        sender msg "&7Un-trusted nation &c$nationName&7 from blueprint $name"
    }
}
