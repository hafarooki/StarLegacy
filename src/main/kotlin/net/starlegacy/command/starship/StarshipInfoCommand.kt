...
object StarshipInfoCommand : SLCommand() {
    ...
    fun onExecute(p: Player) {
        ...
        val powerOutput = ship.powerSource.output
        p msg "   &7Power Output:&f $powerOutput"
        p msg "   &7Weapon Capacitor Capacity:&f ${ship.powerSource.weaponCapacitor.capacity}"
        p msg "   &7Heavy Weapon Booster Output:&f ${ship.powerSource.heavyWeaponBooster.output}"

        p msg "   &7Power Division:"
        val powerTypes = listOf(
            "Shield" to ship.powerSource.powerDistributor.thrusterPortion,
            "Weapon" to ship.powerSource.powerDistributor.weaponPortion,
            "Thruster" to ship.powerSource.powerDistributor.thrusterPortion
        )
    ...
    private fun createPercent(numerator: Int, denominator: Int): String = createPercent(numerator.toDouble() / denominator.toDouble())
